...
class CarsViewset(APIView):
    def get(self, request, id=None):
        if id:
            item = models.Cars.objects.get(id)
    ...
    def patch(self, request, id=None):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"status": "success", "data": seializer.data}, status=status.HTTP_200_OK)
        else:    
            return Response ({"status": "error", "data": seializer.errors}, status=status.HTTP_400_BAD_REQUEST)
